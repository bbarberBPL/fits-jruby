...
- RVM installed system-wide or for the service user.
...
Create a dedicated system account. The account is given a home directory under
`/var/fits` (not under `/home`) to host the per-user RVM installation; it still
...
# RVM wrapper: generated by `rvm wrapper jruby-9.4.15.0` (run as the fits user).
# It bakes in GEM_HOME/GEM_PATH/PATH, so systemd needs no login shell
# (no `bash -lc` / `rvm-exec` layer) — NoNewPrivileges=yes and the rest of
# the hardening are unaffected.
...
# ProtectHome=read-only governs /home, /root, and /run/user — it does NOT
# govern /var/fits. It is ProtectSystem=strict (above) that makes /var/fits
# readable-but-not-writable, which is exactly what the RVM wrapper needs
# (read-only gem tree). Do NOT add /var/fits to ReadWritePaths.
...
These are inherent to the current design — they are documented here, not
controlled by configuration knobs.

1. **Shutdown grace.** On `systemctl stop` (SIGTERM), the server stops
   accepting connections, then waits up to 5 seconds for an in-flight
   examination to finish before killing the worker thread and exiting cleanly
   (exit 0). This 5 s bound is set by the server itself (`Thread#join(5)` in
   the worker drain) — NOT by systemd's `TimeoutStopSec`. Tuning
   `TimeoutStopSec` in the unit will not change it. The kill is bounded (an
   examination cannot block shutdown forever) and visible in the journal.
2. **Serial worker.** One examination runs at a time by design; FITS is not
   thread-safe and parallel workers would multiply heap cost. A large file blocks
   the queue for its duration. Idle or slow clients are bounded by
   `FITS_READ_TIMEOUT` (default 5 s), which disconnects clients that connect but
   never send a request line, keeping the worker free.
3. **Installer redirects.** The build-time `bin/setup` installer follows up to 5
   https redirects when fetching the FITS zip. Every redirect must stay on https
   (http redirects are refused). The downloaded archive is integrity-checked
   against the pinned SHA-256 before extraction.
...
| `requests_error / requests_total` ratio rising | Examination failures or protocol/validation errors are rising | Check `journalctl -u fits.service` for `ERROR:` lines; verify file permissions and format support |
